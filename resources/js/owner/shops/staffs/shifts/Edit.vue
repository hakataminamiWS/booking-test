<template>
    <OwnerLayout :shop="props.shop" currentPage="shifts">
        <v-container>
...
interface Schedulable {
    workable_start_at: string;
    workable_end_at: string;
}
...
const formAction = computed(
    () => `/owner/shops/${props.shop.slug}/staffs/${props.staff.id}/shifts`
);
...
const weekSchedules = ref<DaySchedule[]>(
    weekDays.map((date) => {
        ...
        const existingSchedules = props.schedules
            ...
            .filter((s) => {
                // workable_start_at (UTC) を店舗のタイムゾーンに変換し、その日付部分がdateStrと一致するかチェック

                const localDateStr = formatInTimeZone(
                    s.workable_start_at,
                    props.shop.timezone ?? "Asia/Tokyo",
                    "yyyy-MM-dd"
                );
        ...
        // 休日判定ロジック: 00:00-00:00のスケジュールが1つだけ存在する場合を休日とする
...
const addSchedule = (dayIndex: number) => {
    ...
    let newStartTime = "09:00"; // フォールバック

    let newEndTime = "18:00"; // フォールバック
    ...
    // 休日状態から時間を追加する場合、00:00-00:00のダミーデータは不要なので削除する
...
const setAsHoliday = (dayIndex: number) => {
    // DBスキーマに合わせて、00:00-00:00のスケジュールを登録
