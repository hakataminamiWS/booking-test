                ...
                <v-card>
                    <v-card-title
                        ...
                        <v-btn
                               ...
                               :href="`/owner/shops/${props.shop.slug}/menus/create`"
                    ...
                    <v-card-text>
                        ...
                        <v-data-table-server
                            ...
                            <template v-slot:item.requires_staff_assignment="{ item }">
                                <v-chip
                                        :color="item.requires_staff_assignment
                                            ? 'blue'
                                            : 'grey'
                                            "
                                        dark
                                        small>
                                    {{
                                        item.requires_staff_assignment
                                            ? "必須"
                                            : "不要"
                                    }}
                                </v-chip>
                            ...
                            <template v-slot:item.cancellation_deadline_minutes="{
                                item,
                            }">
                                {{
                            ...
                            <template v-slot:item.actions="{ item }">
                                <v-btn
                                       color="primary"
                                       :href="`/owner/shops/${props.shop.slug}/menus/${item.id}/edit`">編集する
                                </v-btn>
                            </template>
                        </v-data-table-server>
                    </v-card-text>
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-row v-for="filter in filters" :key="filter.id" align="center">
                        ...
                        <v-col cols="7">
                            <div v-if="getColumnType(filter.column) === 'range'" class="d-flex align-center">
                                <v-text-field v-model="filter.value" @update:model-value="
                                    filter.value =
                                    formatNumericInput($event)
                                    " label="開始値" dense hide-details class="mr-2" inputmode="numeric"></v-text-field>
                                <span>-</span>
                                <v-text-field v-model="filter.value_to" @update:model-value="
                                    filter.value_to =
                                    formatNumericInput($event)
                                    " label="終了値" dense hide-details class="ml-2" inputmode="numeric"></v-text-field>
                            </div>
...
const filterableColumns = ref([
    { text: "メニュー名", value: "name", type: "text" },
    { text: "価格", value: "price", type: "range" },
    { text: "所要時間", value: "duration", type: "range" },
    {
        text: "担当者必須",
        value: "requires_staff_assignment",
        type: "select",
        items: [
            { title: "必須", value: "1" },
            { title: "不要", value: "0" },
...
const activeFiltersText = computed(() => {
    return activeFilters.value.map((f) => {
        ...
        } else if (f.column === "requires_staff_assignment") {
            const item = getColumnItems(f.column).find(
                (i) => i.value === f.value
            );
            displayValue = item ? item.title : f.value;
        }
...
const sortableColumns = ref([
    { text: "メニュー名", value: "name" },
    { text: "価格", value: "price" },
    { text: "所要時間", value: "duration" },
    { text: "担当者必須", value: "requires_staff_assignment" },
    { text: "特別キャンセル期限", value: "cancellation_deadline_minutes" },
    { text: "特別予約締切", value: "booking_deadline_minutes" },
]);
...
const loadItems = async (options: Options) => {
    ...
    history.pushState(
        null,
        "",
        `/owner/shops/${props.shop.slug}/menus?${params.toString()}`
    );
...
const headers: Headers = [
    { title: "メニュー名", key: "name", sortable: false },
    { title: "価格", key: "price", sortable: false },
    { title: "所要時間", key: "duration", sortable: false },
    { title: "担当者必須", key: "requires_staff_assignment", sortable: false },
    { title: "担当スタッフ", key: "staffs", sortable: false },
    { title: "関連オプション", key: "options", sortable: false },
    {
        title: "特別キャンセル期限",
        key: "cancellation_deadline_minutes",
        sortable: false,
    },
    {
        title: "特別予約締切",
        key: "booking_deadline_minutes",
        sortable: false,
    },
