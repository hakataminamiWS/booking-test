...
const { getContractStatusText } = useContractStatus();
...
const updateStartDateFromPicker = (date: any) => {
    if (!date) return;
    form.value.start_date = date.toISOString().slice(0, 10);
    startDateDialog.value = false;
};

const updateEndDateFromPicker = (date: any) => {
    if (!date) return;
