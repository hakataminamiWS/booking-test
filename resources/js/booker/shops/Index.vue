                    ...
                    <v-card-text>
                        ...
                        <v-data-table-server
                            ...
                            <template v-slot:item.last_booking_at="{ item }">
                                {{ item.last_booking_at ? new Date(item.last_booking_at).toLocaleDateString() : '-' }}
                            </template>
                            <template v-slot:item.booking_count="{ item }">
                                {{ item.booking_count ?? 0 }} 回
                            </template>
                            <template v-slot:item.actions="{ item }">
                                <v-btn
                                       color="primary"
                                       size="small"
                                       :href="`/shops/${item.shop_slug}/booker/profile/edit`"
                                       class="mr-1">
                                    プロフィール
                                </v-btn>
                                <v-btn
                                       color="secondary"
                                       size="small"
                                       :href="`/shops/${item.shop_slug}/booker/bookings`"
                                       class="mr-1">
                                    予約履歴
                                </v-btn>
                                <v-btn
                                       color="success"
                                       size="small"
                                       :href="`/shops/${item.shop_slug}/booker/bookings/create`">
                                    新規予約
                                </v-btn>
                            </template>
                        </v-data-table-server>
...
const sortableColumns = ref([
    { text: "店舗名", value: "name" },
    { text: "最終予約日", value: "last_booking_at" },
...
const loadItems = async (options: Options) => {
    ...
    const apiUrl = `/booker/api/shops?${params.toString()}`;
    history.pushState(null, "", `/booker/shops?${params.toString()}`);
...
const headers: Headers = [
    { title: "店舗名", key: "shop_name", sortable: false },
    { title: "最終予約日", key: "last_booking_at", sortable: false },
    { title: "予約回数", key: "booking_count", sortable: false },
