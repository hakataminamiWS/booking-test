            ...
            <v-col cols="12">
                <v-card>
                    <v-card-title
                        ...
                        <v-btn
                               color="primary"
                               :href="`/shops/${shop.slug}/staff/bookings/create`"
                    ...
                    -->
                        ...
                        <v-data-table-server
                            ...
                            <template v-slot:item.actions="{ item }">
                                <v-btn
                                       color="primary"
                                       :href="`/shops/${shop.slug}/staff/bookings/${item.id}/edit`">
                                    編集する
                                </v-btn>
                            </template>
                        </v-data-table-server>
                ...
                <v-card-text>
                    <v-row v-for="filter in filters" :key="filter.id" align="center">
                        ...
                        <v-col cols="7">
                            ...
                            <v-text-field v-if="getColumnType(filter.column) === 'number'" v-model="filter.value"
                                          type="number"
                                          label="値" dense hide-details></v-text-field>
                            <v-text-field v-if="getColumnType(filter.column) === 'date'" v-model="filter.value"
                                          type="date"
                                          label="日付" dense hide-details></v-text-field>
                            <div v-if="getColumnType(filter.column) === 'date-range'">
                                <FilterDateRangePicker v-model:value="filter.value" v-model:valueTo="filter.value_to" />
                            </div>
...
const loadItems = async (options: Options) => {
    ...
    const apiUrl = `/shops/${props.shop.slug}/staff/api/bookings?${params.toString()}`;
    history.pushState(null, "", `/shops/${props.shop.slug}/staff/bookings?${params.toString()}`);
