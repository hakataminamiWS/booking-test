...
会員が特定の店舗における自身のステータス（最終予約日、予約回数など）を確認し、各機能（プロフィール編集、予約履歴、新規予約）へアクセスするためのダッシュボード画面です。

#### 画面仕様詳細 (`/shops/{shop}/booker`)
...
-   **最終予約日**: `shop_bookers_crm.last_booking_at`
-   **来店回数**: `shop_bookers_crm.booking_count`

メニューリスト：

以下の機能へのナビゲーションリンクを表示します。

| ラベル             | 遷移先                                       | 説明                                   |
| :----------------- | :------------------------------------------- | :------------------------------------- |
| **プロフィール編集** | `/shops/{shop}/booker/profile/edit`          | 名前、連絡先の確認・変更を行います。   |
| **予約履歴**       | `/shops/{shop}/booker/bookings`              | 過去の予約や今後の予定を確認します。   |
| **新規予約**       | `/shops/{shop}/booker/bookings/create`       | 新しい予約を行います。                 |

#### バックエンド仕様

##### データ受け渡し (ページ表示)

-   **ルート**: `GET /shops/{shop:slug}/booker`
-   **コントローラ**: `App\Http\Controllers\Booker\ShopController@show`
-   **処理内容**:
    -   `ShopBookerPolicy@view` 等で認可チェックを行います。
    -   対象の `Shop` およびログインユーザーの `ShopBooker` (CRM情報含む) を取得します。
    -   Vue コンポーネントへデータを渡し、Blade ビューを表示します。
...
会員が特定の店舗における自身の会員情報を削除（退会）するための機能です。
退会後も過去の予約履歴は保持されます（イベント情報として記録を残すため）。

#### 画面仕様詳細

退会リンクは、店舗マイページ (`/shops/{shop}/booker`) のカードアクション部分に配置します。
メニューリストとは別のセクションとして表示します。

##### 退会確認ダイアログ

- リンクまたはボタンをクリックすると確認ダイアログを表示します。
- ダイアログには以下を表示します:
  - 警告メッセージ: 「退会すると、この店舗での会員情報が削除されます。過去の予約履歴は保持されます。」
  - 自身の会員番号の表示
  - 会員番号入力フィールド（確認用）
- 退会を確定するには、表示されている会員番号を入力する必要があります（誤操作防止）。

#### バックエンド仕様

##### データ受け渡し (退会処理)

- ルート: DELETE /shops/{shop:slug}/booker/profile
- コントローラ: App\Http\Controllers\Booker\ProfileController@destroy
- リクエストクラス: App\Http\Requests\Booker\DestroyProfileRequest

##### バリデーション

- バックエンド (DestroyProfileRequest):
  - 認可: ShopBookerPolicy@delete を呼び出し、自身の会員情報のみ削除可能であることを確認します。
  - ルール:
    - member_number: required, 入力された会員番号が自身の会員番号と一致すること。

##### 処理内容

1. DestroyProfileRequest で認可とバリデーションを実行します。
2. バリデーションが成功した場合、以下の処理を実行します:
   - bookings テーブルの shop_booker_id を NULL に更新（当該会員の予約を対象）
   - shop_bookers レコードを物理削除（shop_bookers_crm は CASCADE で自動削除）
3. 削除後、登録店舗一覧画面 (`/booker/shops`) にリダイレクトし、「退会しました」という成功メッセージを表示します。

##### 外部キー制約への対応

| テーブル | カラム | 削除時の対応 |
|:---|:---|:---|
| bookings | shop_booker_id | NULL に更新（過去の予約履歴は保持） |
| shop_bookers_crm | shop_bookers_id | CASCADE で自動削除 |
