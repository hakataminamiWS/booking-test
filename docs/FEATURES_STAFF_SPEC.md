...
ログイン後のトップページとして、店舗運営に関する重要情報を集約して表示します。オーナーのダッシュボード (`/owner/dashboard`) と類似していますが、スタッフ向けの権限制御が適用されます。

#### 画面仕様詳細 (`/shops/{shop}/staff/dashboard`)
...
1.  **本日の予約**
    -   オーナーダッシュボードと同様に、当日の予約をタイムライン形式で表示します。
    -   自身の担当予約だけでなく、店舗全体の予約を表示します（チーム連携のため）。
    -   詳細ボタンから予約詳細・編集画面へ遷移可能です。

2.  **本日のキャンセル**
    -   当日のキャンセル数を表示します。

3.  **シフト登録状況**
    -   シフト未登録のスタッフがいる場合、警告を表示します。
    -   **自身（ログイン中のスタッフ）のチップ**: クリック可能。自身のシフト編集画面 (`/shops/{shop}/staff/shifts/edit`) へ遷移します。
    -   **他スタッフのチップ**: クリック不可（または遷移なし）。他者のシフト編集はできません。

##### バックエンド仕様

-   **ルート**: `GET /shops/{shop:slug}/staff/dashboard`
-   **コントローラ**: `App\Http\Controllers\Staff\DashboardController@index`
-   **データ**:
    -   `summary`: 当日の予約総数など。
    -   `bookings`: 当日の予約リスト。
    -   `cancellations`: 当日のキャンセル情報。
    -   `shifts`: シフト登録状況。`unregistered_staff` 配列には、スタッフIDと名前を含めます。フロントエンドで自身のIDと比較し、リンクの有効化/無効化を判定します。
