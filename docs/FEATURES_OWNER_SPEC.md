...
-   **シフト一覧画面 (`/owner/shops/{shop}/shifts`)**: 店舗に所属する全スタッフの週間シフト登録状況を一覧で確認します。
-   **シフト登録・編集画面 (`/owner/shops/{shop}/staffs/{staff}/shifts`)**: 特定のスタッフの週間シフトを登録・編集します。

### メニュー管理

-   **メニュー一覧画面 (`/owner/shops/{shop}/menus`)**: 店舗で提供するメニューとオプションを一覧で確認します。
-   **メニュー登録画面 (`/owner/shops/{shop}/menus/create`)**: メニューやオプションの詳細を登録・編集します。
    -   [メニュー登録画面](./FEATURES_OWNER_MENU_SPEC.md)
...
| **プロフィール画像** | テキスト入力 | `image_url` | URL 形式。ファイルアップロードにより登録。 |
...
オーナーが店舗に所属する全スタッフの週間シフト登録状況を一覧で確認するための画面です。
各スタッフが週ごとにシフトを入力済みかどうかが一目で分かり、未入力または確認したいセルから直接、対象スタッフ・対象週のシフト編集画面へ遷移できます。

#### 画面仕様詳細 (`/owner/shops/{shop}/shifts`)

-   **ウィンドウタイトル**: `シフト一覧`
-   **クエリパラメータ**: `?date=YYYY-MM-DD` (指定された日付を含む月を基準に週を表示。なければ今月)

##### 表示項目一覧

**リンクセクション：**

-   「スタッフ一覧に戻る」のリンクをページ上部に表示します。
...
-   **月移動コントロール**:
    -   対象月の表示 (例: `2025年11月`)
    -   「< 前の月へ」「次の月へ >」ボタンを配置し、表示する週の範囲を月単位で移動できるようにします。
-   **シフト状況テーブル**:
    -   縦軸にスタッフのニックネーム、横軸に週の期間を表示するテーブルです。
    -   横軸の週は、対象月の第 1 週から 5〜6 週間分をカレンダーに合わせて表示します。

| スタッフ       | 11/02 (日) 〜 11/08 (土)                                                                                                       | 11/09 (日) 〜 11/15 (土)                                                                                                       | ... |
| :------------- | :----------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------- | :-- |
| **スタッフ A** | <a href="/owner/shops/{shop}/staffs/{staffA}/shifts?date=2025-11-02" style="color: green; text-decoration: none;">入力済み</a> | <a href="/owner/shops/{shop}/staffs/{staffA}/shifts?date=2025-11-09" style="color: red; text-decoration: none;">未入力</a>     | ... |
| **予約枠 B**   | <a href="/owner/shops/{shop}/staffs/{staffB}/shifts?date=2025-11-02" style="color: green; text-decoration: none;">入力済み</a> | <a href="/owner/shops/{shop}/staffs/{staffB}/shifts?date=2025-11-09" style="color: green; text-decoration: none;">入力済み</a> | ... |
| ...            | ...                                                                                                                            | ...                                                                                                                            | ... |

-   **セル**:
    -   各セルには「入力済み」または「未入力」のテキストを表示します。
    -   セル全体が、そのスタッフのその週のシフト編集画面へのリンクになります。
    -   リンク先: `/owner/shops/{shop}/staffs/{staff}/shifts?date={週の開始日}`
    -   「入力済み」は緑色、「未入力」は赤色で表示し、視認性を高めます。

#### バックエンド仕様

##### データ受け渡し (ページ表示)

-   **ルート**: `GET /owner/shops/{shop:slug}/shifts`
-   **コントローラ**: `App\Http\Controllers\Owner\ShopStaffScheduleController@index` (新規作成)
-   **処理内容**:
    1.  `ShopPolicy@view` で認可チェックを行います。
    2.  クエリパラメータ `date` を基に対象月を決定します（デフォルトは当月）。
    3.  対象月の週の範囲（例: 5 週間分）を計算し、テーブルヘッダー用の日付データを作成します。
    4.  対象店舗に所属する全スタッフ (`shop_staffs` と `profiles`) を取得します。
    5.  各スタッフと各週の組み合わせについて、`shop_staff_schedules` テーブルに該当期間のレコードが存在するかをチェックします。
    6.  以下の構造のデータをビューに渡します。
        -   `shop`: 店舗情報
        -   `weekHeaders`: テーブルのヘッダーとなる週ごとの期間情報
        -   `staffsWithShiftStatus`: スタッフ情報と、各週のシフト入力状況（`'entered'` or `'not_entered'`）を格納した配列
    7.  `owner.shops.staffs.shifts.index` ビューを返します。
...
オーナーが特定のスタッフの週間シフトを登録・編集するための画面です。
店舗の営業情報（通常営業時間、定休日、特別営業日・休業日）をデフォルトとして提案します。
営業時間外のシフト登録や休業日のシフト登録には警告を表示しますが、登録自体は可能とします。

#### 画面仕様詳細 (`/owner/shops/{shop}/staffs/{staff}/shifts`)

-   **ウィンドウタイトル**: `シフト編集: {スタッフ名}`
-   **クエリパラメータ**: `?date=YYYY-MM-DD` (指定された日付を含む週を表示。なければ今週)

##### 表示項目一覧

リンクセクション：

-   「スタッフ一覧に戻る」のリンクをページ上部に表示

店舗・スタッフヘッダー：

-   店舗：{店舗名} (店舗 ID: {店舗 ID}) / スタッフ: {スタッフニックネーム}

週移動コントロール：

-   対象週の表示 (例: `2025/10/26 (日) 〜 2025/11/01 (土)`)
-   「< 前の週へ」「次の週へ >」ボタン
-   日付ピッカーによるジャンプ機能
...
-   フォーム全体は `<v-card>` で囲みます。
-   カードタイトルは「シフト編集」とします。
-   カードの最下部に「この内容で保存する」ボタンを配置します。
-   7 日分（日〜土）のシフト入力欄をテーブル形式で表示します。

##### フォーム項目一覧 (曜日ごとの行)

| ラベル/要素      | UI           | `name`属性 (例: 月曜日)                                    | 備考                                                            |
| :--------------- | :----------- | :--------------------------------------------------------- | :-------------------------------------------------------------- |
| **日付・曜日**   | テキスト     | -                                                          | `10/27 (月)` のように表示。                                     |
| **店舗営業情報** | テキスト     | -                                                          | `定休日`, `09:00-18:00` など参考情報を表示。                    |
| **時間入力**     | 時間入力ペア | `schedules[1][N][start_time]`, `schedules[1][N][end_time]` | 複数登録可能。横に削除(x)ボタン。                               |
| **警告**         | アイコン     | -                                                          | 営業時間外・休業日登録時に `(!)` アイコンとツールチップで警告。 |
| **アクション**   | ボタン       | -                                                          | 「時間を追加」「休日にする」ボタン。                            |

#### バックエンド仕様

##### データ受け渡し (ページ表示)

-   **ルート**: `GET /owner/shops/{shop:slug}/staffs/{staff}/shifts`
-   **コントローラ**: `App\Http\Controllers\Owner\ShopStaffScheduleController@edit`
-   **処理内容**:
    -   `ShopStaffPolicy` (または `ShopStaffSchedulePolicy`) で認可チェック。
    -   ルートモデルバインディングで `Shop` と `ShopStaff` を取得。
    -   クエリパラメータ `date` を基に対象週を決定。
    -   対象週の既存シフト (`shop_staff_schedules`) と店舗営業情報 (`shop_business_hours_regulars`, `shop_special_open_days`, `shop_special_closed_days`) を取得。
    -   取得した全データを `owner.shops.staffs.shifts.edit` ビューに渡す。
    -   CSRF トークン等を Blade から Vue へ渡す。

##### フォーム送信 (更新処理)

-   **ルート**: `PUT /owner/shops/{shop:slug}/staffs/{staff}/shifts`
-   **コントローラ**: `App\Http\Controllers\Owner\ShopStaffScheduleController@update`
-   **リクエストクラス**: `App\Http\Requests\Owner\UpdateShopStaffScheduleRequest`

##### バリデーション (`UpdateShopStaffScheduleRequest`)

-   **認可**: `authorize` メソッド内で認可チェック。
-   **ルール**:
    -   `schedules`: `present`, `array`
    -   `schedules.*`: `array` (曜日ごとの配列)
    -   `schedules.*.*.start_time`: `required`, `date_format:H:i`
    -   `schedules.*.*.end_time`: `required`, `date_format:H:i`, `after:schedules.*.*.start_time`
    -   _注意: 登録をブロックするロジックは最小限とし、柔軟性を重視する。_

##### 処理内容

1. `UpdateShopStaffScheduleRequest` で認可とバリデーションを実行。
2. トランザクション内で、対象スタッフの対象週の既存シフトを全て削除 (`delete`)。
3. リクエストで受け取った新しいシフト情報を `createMany` などで一括登録。
4. 更新後、同じシフト編集画面にリダイレクトし、「シフトを更新しました」という成功メッセージを表示する。

---

### 4.3. メニュー管理

ドキュメントを分ける。AI に読み込む際に、ノイズが混じらないように。
