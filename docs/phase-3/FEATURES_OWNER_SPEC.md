...
-   **スタッフ招待画面 (`/owner/shops/{shop}/staff-invitations/create`)**: 新しいスタッフをメールで店舗に招待します。
...
-   **スタッフ詳細管理画面 (`/owner/shops/{shop}/staffs/{staff}/edit`)**: 店舗に所属するスタッフの詳細情報を編集します。
...
-   **共通ヘッダー**: 「店舗ヘッダー」コンポーネントを表示します。
-   **ボタン**:
    -   **詳細画面に戻るボタン**: `/owner/shops/{shop}` へ遷移します。
-   **フォーム**:
...
**(今後、このセクションにスタッフ管理関連の画面仕様を記述していきます)**
