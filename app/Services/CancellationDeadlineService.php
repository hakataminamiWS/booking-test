...
class CancellationDeadlineService
{
    ...
    public function calculate(Shop $shop, ?ShopMenu $menu, Carbon $startAt): Carbon
    {
        ...
        // 優先設定: メニュー個別設定
        if ($menu && $menu->requires_cancellation_deadline && $menu->cancellation_deadline_minutes !== null) {
    ...
    {
        $deadline = $this->calculate($shop, $menu, $startAt);

        if ($shop->timezone) {
            $deadline->setTimezone($shop->timezone);
        }
